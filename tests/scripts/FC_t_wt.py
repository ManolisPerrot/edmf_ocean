    ...
    for i, label in enumerate(run_label):
        if run_label == 'ED':
            ...
            ax.plot(-(scm[i].wted + scm[i].wtmf), scm[i].z_w/mld, linestyle=styles[i], color = colors[i],
                    alpha=alpha[i], linewidth=linewidth[i], label=label)
            ax.plot( -scm[i].wtmf, scm[1].z_w/mld, color =colors[i] , linestyle =':', alpha=alpha[i] , linewidth=2 )
...
fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(
        0.5, 0.15), fancybox=False, shadow=False, ncol=4)
