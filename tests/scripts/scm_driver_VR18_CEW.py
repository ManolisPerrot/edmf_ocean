...
###########################################
case        = 'Van_Roekel_2018_CEW'
saving_path = '../figures/'
saving_name = case+'72h_profile_VR18_CEW.png'
###########################################
end_time   = 72
#===========================================================================
compar_les = 0
if compar_les==1:
  ...
  les_file   = les_dir+"convectSaltWind_av.nc"
  ...
  u_les     = fh1.variables['U'][indx_les,:]  # zm
  v_les     = fh1.variables['V'][indx_les,:]  # zm
...
for i, label in enumerate(run_label):
  ax.plot(scm[i].akv_avg[:],vscale*scm[i].z_w, linestyle=styles[i], color = colors[i],linewidth=linewidth[i])
#if compar_les==1:
...
for i, label in enumerate(run_label):
  ax.plot(scm[i].u_avg[:],vscale*scm[i].z_r, linestyle=styles[i], color = colors[i],linewidth=linewidth[i])
if compar_les==1:
  ax.plot(  u_les[:],zr_les*vscale, linewidth=2, color='r'  )
#==
...
for i, label in enumerate(run_label):
  ax.plot(scm[i].v_avg[:],vscale*scm[i].z_r, linestyle=styles[i], color = colors[i],linewidth=linewidth[i])
if compar_les==1:
  ax.plot(  v_les[:],zr_les*vscale, linewidth=2, color='r'  )
#==
ax.set_xlabel(r"${\rm m}\;{\rm s}^{-1}$",fontsize=14)
ax.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
#===============================
