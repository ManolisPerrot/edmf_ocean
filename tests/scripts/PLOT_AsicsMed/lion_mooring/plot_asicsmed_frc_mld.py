...
class Lion_data:
    ...
    def get_data(self,OBS):
        ...
        if OBS:
            self.temp = np.flip( np.delete(temp, [0,1,25], axis=1), axis=1 )
            self.z1d  = np.flip( np.delete(zz, [0,1,25], axis=0), axis=0 )
        else:
            self.temp = temp
            self.z1d  = zz
...
data_params = {'filename': '../data/LION_mooring_data.nc', 'time_var_name': 'ocean_time',
               'time_end': 598,'data_title':'Lion mooring data'}
...
ax.plot(scm_data.time, gaussian_filter(scm_data.hmxl,sigma=3),color='k',linewidth=1.5,label=scm_data.config["data_title"])
...
plt.tight_layout()
plt.savefig('../figures/LION_MOORING_frc_hmxl.pdf', bbox_inches='tight')
#===========================================================================
