...
saving_path = '../figures/'
saving_name = case+'72h_wtke_sensitivity.png'
...
# run_label = [r'TKE+EVD',r'Witek et al. (2011)', r'Han \& Bretherton (2019) ',r'EDMF-Energy']
run_label = [r'Witek et al. (2011)', r'Han \& Bretherton (2019) ',r'EDMF-Energy']
runs = [
    # {
    #     'eddy_diff': True,
    #     'evd': True,
...
# ,
    {
        ...
        'mass_flux_tke': True,
        'mass_flux_tke_trplCorr': True,
        'entr_scheme': 'R10_Wi11',
        'output_filename': 'Witek.nc'
    },
        {
        'eddy_diff': True,
        'evd': False,
        ...
        'output_filename': 'HanBretherton.nc'
    },
    {
        'eddy_diff': True,
        'evd': False,
...
# ===============================================================
    ...
    ax.plot(TH_les[instant], z_r_les/mld, style_les,
            alpha=alpha_les, markersize=markersize_les,  label='LES')
...
#add velocity-pressure correlation
    ...
    ax.plot(-WU[instant], z_r_les/mld, style_les,alpha=alpha_les, markersize=markersize_les, label='LES')
...
#add velocity-pressure correlation
    ...
    ax.plot((-akv[1:]*(tke[1:]-tke[:-1])/dz), scm[i].z_r/mld,  
                    #  marker='o',    linestyle='', markersize=markersize_scm,
                         color='C8',
                         label=r'$-K_k \partial_z k$')
    
    ax.plot((ap*wp*(tke_p-tke)), scm[i].z_w/mld,
            # marker='+', linestyle='', markersize=markersize_scm, 
            color='tab:brown',label=r'$a_p w_p (k_p - k)$')
    ax.plot((ap[1:]*wp[1:]*(up[1:]-u)**2/2), scm[i].z_r/mld,  
            # marker='>', linestyle='', markersize=markersize_scm, 
            color='C6',
            label=r'$a_p w_p \frac{\|\mathbf{u}_{h,p} - \overline{\mathbf{u}}_h \|^2 }{2} $')
    ax.plot((ap*wp**3 *1/2), scm[i].z_w/mld,  
            # marker=(5,1), linestyle='', markersize=markersize_scm,
            color='C9',
                label=r'$a_p w_p \frac{w_p^2}{2} $')

    handles, labels = ax.get_legend_handles_labels()
    axes.flat[-1].legend(handles,labels, fancybox=False, shadow=False,)
    ax.set_xlim((- 1.5e-5, 2e-6))
    ax.set_ylim((-1.3, 0))
    ...
    fig.legend(handles,labels, loc='lower right', bbox_to_anchor=(
        1., 0.17),fancybox=False, shadow=False,)
