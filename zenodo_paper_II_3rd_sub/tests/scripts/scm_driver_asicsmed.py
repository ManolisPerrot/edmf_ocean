...
# tdtm  = np.array([230,460,620]) #original tdm from Florian (error??? since 230/10 = 23, 460/20 = 23 BUT 620/23 != 23)

tscm  = np.array([10+2,20,25]);tscm  = tscm*24
tdtm  = tscm/24*23
...
for i in range(len(runs)):
  ...
  out[i] = Dataset(runs[i]['output_filename'], mode='r')
#
for x in range(nd):
  ntime     = tscm[x]
  ntime_dta = tdtm[x]
...
# out[0]    = Dataset(out[0].output, mode='r')
out[0]    = Dataset(runs[0]['output_filename'], mode='r')
ustar     = np.sqrt(np.sqrt( np.power((out[0].rho0/1.22)*out[0].variables['taux'][time1:time2],2)
                           + np.power((out[0].rho0/1.22)*out[0].variables['tauy'][time1:time2],2) ))
time      = out[0].variables['ocean_time'][time1:time2] / (3600*24)
# data to compute B0
Qsol      = out[0].variables['Qs'][time1:time2] /(out[0].rho0*out[0].cp)   #K m/s
Qnsol     = out[0].variables['Qns'][time1:time2]/(out[0].rho0*out[0].cp)   #K m/s
...
Buoy_sfc = rho0*(alpha*(Qsol+Qnsol)-beta*Fw)
#==================================================================
# tscm  = np.array([20,40,51]);tscm  = tscm*12  # draw vertical blue lines on the plot
# tscm  = tscm/24;tscm  = tscm*12  # draw vertical blue lines on the plot
tscm = tscm.astype(int) #convert to integers
#===============================================
...
for tt in range(ntime):
    ...
    rho,bvf = scm_oce.rho_eos(temp,salt,z_r,z_w,rho0,nz)
    drho    = 0.01; zref = z_r[-5]
    hmxl_dta[tt] = scm_oce.compute_mxl2(bvf,0.01,z_r,-300.,rho0,nz)
#========================================================
#### create figure
...
for i,ax in enumerate(axes.flat):
    ax.text(0.05, 0.97, subplot_label[i], transform=ax.transAxes, bbox=dict(facecolor='1.', edgecolor='none',alpha=0.5), fontweight='bold', va='top', ha='right')
