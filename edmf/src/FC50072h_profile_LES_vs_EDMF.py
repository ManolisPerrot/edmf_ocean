...
TKE = (LG_RES.RES_KE + LG_SBG.SBG_TKE).data
...
if case == 'FC500':
    ...
    for i, label in enumerate(run_label):
        # ax.plot(out[i].temp[instant], out[i].z_r/mld, styles[i], color = colors[i], alpha=alpha[i], linewidth = linewidth[i], label = label )
        ax.plot(scm[i].t_np1[:, 0], out[i].z_r/mld, styles[i], color = colors[i],
                alpha=alpha[i], linewidth=linewidth[i], label=label)
...
plt.savefig(saving_path+'FC50072h_profile_LES_vs_EDMF.png', bbox_inches='tight', dpi=300)

plt.show()

print('figure saved at'+saving_path+'FC50072h_profile_LES_vs_EDMF.png')
